...
tmux split-window -h -d 'python ../scripts/rate_thrust_publisher.py' #1 rate publisher
sleep 1

tmux select-pane -t 2
#tmux split-window -h -d 'roslaunch flightgoggles core.launch ignore_collisions:=1' #2 simulator
tmux split-window -h -d 'roslaunch flightgoggles core.launch render_stereo:=1' #2 simulator
sleep 1
